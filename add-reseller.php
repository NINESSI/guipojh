		...
		if($deviceschecked > 1) {
			...
			if ($deviceschecked == 1) {
				if ($dados_logado['type'] == "reseller") {
					$query = mysqli_query($con, "INSERT INTO `users` (`username`,`password`,`registered`,`expired`,`reseller`,`type`,`credits`) VALUES ('$userchecked','$passchecked','$date','$endatechecked','$vendedorchecked','cliente','0')");
...
<body>
		...
		<div class="app-main__outer">
            <div class="app-main__inner">
				...
				<div class="col-md-12">
					...
					<a href="index_reseller" class="btn btn-dark"><i class="fa fa-arrow-left"></i>  VOLTAR</a>
					<?php } else { ?>
						<a href="index" class="btn btn-dark"><i class="fa fa-arrow-left"></i>  VOLTAR</a>
					<?php } ?>
				</div>			
				</div>
				<div class="row">
                    <div class="col-md-12">
                        <div class="main-card mb-3 card">
                            <div class="card-header">WTP - ADICIONAR Vendedor
                            </div>
                            <div class="card-body">
								...
								<form action="" method="POST">
    								<div class="position-relative form-group">
        								...
        								<input type="text" name="usuario" placeholder="" class="form-control" required>
    								</div>
    								
    								<div class="position-relative form-group">
        								<label for="exampleEmail" class="">Senha</label>
        								<input type="text" name="senha" placeholder="" class="form-control" required>
    								</div>
    								
    								<div class="position-relative form-group">
        								<label for="exampleEmail" class="">Devices</label>
        								<input type="number" max="1" min="1" name="devices" value="1" class="form-control" readonly required>
    								</div>
    								...
    								<div class="position-relative form-group">
        								<label for="exampleEmail" class="">Hora de Registro</label>
        								<input size="16" type="text" name="startdate" value="<?php echo date("Y-m-d h:i"); ?>" readonly class="form-control">
    								</div>
    								
    								<div class="position-relative form-group">
        								<label for="exampleEmail" class="">Plano</label>
        								<select class="form-control" name="endate">
    								    <?php if ($dados_logado['type'] == "reseller") { ?>
    								        <option value="<?php echo Date('Y-m-d h:i', strtotime('+365 days')); 
    								        ?>" selected>365 days</option>
								        <?php } else { ?>
							                <option value="<?php echo Date('Y-m-d h:i', strtotime('+365 day')); ?>" >365  Dias</option>
            								<option value="<?php echo Date('Y-m-d h:i', strtotime('+365 days')); ?>" selected>365 Dias</option>
            							<?php } ?>
        								</select>
    								</div>
    								...
    								<div class="position-relative form-group">
        								...
        								<select name="cargo" class="form-control" readonly>
            								<?php if ($dados_logado['type'] == "reseller") { ?>
	...
	<?php if ($dados_logado['type'] == "admin") { ?>
    								...
    								<div class="position-relative form-group">
        								<label for="exampleEmail" class="">Creditos</label>
        								<input type="number" min="0" max="999" name="credit" value="10" class="form-control" required>
    								</div>
    								
    									<div class="position-relative form-group">
        								<label for="exampleEmail" class="">Maximo de Tentativas</label>
        								<input type="number" min="5" max="5" name="xilindro" value="5" class="form-control" readonly required>
    								</div>
    								<?php } ?>
    								<div class="d-block text-right card-footer">
    								    <button type="submit" name="Submit" class="btn btn-success btn-lg">CADRASTAR</button>
    								</div>
								</form>
							</div>
                        </div>
