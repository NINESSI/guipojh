...
$dados_editar = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM users WHERE username = '$ID'"));
$check = mysqli_num_rows(mysqli_query($con, "SELECT * FROM users WHERE username = '$ID'"));
...
if (isset($_POST['Submit'])) {
	...
	if ($deviceschecked > 1) {
	  ...
	  if ($deviceschecked == 1) {
	  	if($endatechecked == "" || $endatechecked == "00-00-00 00:00" || $endatechecked == NULL) {
	  		$endatechecked = $dados_editar['expired'];
	  	}
		$query = mysqli_query($con, "UPDATE users SET username = '$userchecked', password = '$passchecked', uid = NULL, type='$cargochecked', credits='$creditchecked' WHERE username = '$ID'");
		if ($query) {
		    ...
		    $_SESSION['acao'] = "UPDATE users SET username = '$userchecked', password = '$passchecked', uid = NULL,  type='$cargochecked', credit='$creditchecked' WHERE username = '$ID'";
		}
	}
  }
}
...
<head>
    ...
    <title>WTP <?php echo $dados_editar['username']; ?> - ADMIN</title>
                            ...
                            <div class="card-body">
								...
								<form action="" method="POST">
    								...
    								<div class="position-relative form-group">
        								...
        								<input type="number" max="1" min="1" name="devices" value="<?php echo "1";  ?>" class="form-control" readonly required>
    								</div>
    								...
    								<button type="submit" name="Reset" class="btn btn-primary btn-lg">Reset UID</button>
